...
| 2026-03-22 | [guarded-wdk-architecture-one-pager.md](./guarded-wdk-architecture-one-pager.md) | guarded-wdk 패키지 아키텍처 정리 | 타입 그래프(44 nodes, 94 edges) + 12개 소스 파일 전체 분석 | 4개 도메인 Aggregate(Policy/Approval/Ledger/Crypto) + 3축(정책평가/승인처리/보호조립) + 3시나리오 |
| 2026-03-22 | [dead-exports-triage.md](./dead-exports-triage.md) | CI dead-exports 126건 분류 + 원인 분석 | 7개 패키지 전체 grep 기반 런타임 사용/index.ts re-export 확인 | A(진짜 dead) 37건, B(타입 gap) 28건, C(공개 API) 61건 + 후속 Phase 제안 |
| 2026-03-22 | [daemon-architecture-one-pager.md](./daemon-architecture-one-pager.md) | daemon 패키지 아키텍처 정리 (재작성) | 타입 그래프(71 nodes, 106 edges) + 16개 소스 파일 전체 분석 | 5개 도메인(Tool/Message/Relay/Cron/Journal) + 3축(AI대화루프/제어채널/자동화인프라) + Store 경계 위반 분석 |
