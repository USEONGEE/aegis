...
WDK의 `registerWallet`은 `config: ConstructorParameters<W>[1]`로 서브클래스의 정확한 config 타입을 추론하므로, factory에서 `wdk.registerWallet(chainKey, wallet.Manager, wallet.config)`를 호출할 때 `wallet.Manager`가 `typeof WalletManager`이면 `config`는 base `WalletConfig | undefined`로 추론된다. 서브클래스의 추가 config 필드의 컴파일타임 정밀도가 사라지지만, 이번 Phase에서 의도적으로 수용한다 (최종 채택 결정 참조).
...
### 결정 1: WalletEntry — base 타입 사용 (Breaking change)
```ts
interface WalletEntry {
  Manager: typeof WalletManager
...
```
서브클래스 전용 config 필드(e.g. `rpcUrl`)의 컴파일타임 표현력이 줄어드는 **의도된 breaking change**. 현재 daemon이 `wallets: {}`로 호출하고 있어 실제 영향 없음. 서브클래스별 config 정밀도가 필요해지면 별도 Phase에서 제네릭 `WalletEntry<W>`로 확장. 단순함 우선 (Primitive First).
...
2. `GuardedAccount` 인터페이스를 `extends IWalletAccount`로 변경
3. `createGuardedMiddleware` 반환 타입을 `(account: IWalletAccount) => Promise<void>`로 변경
4. `signTypedData` 접근을 optional 체크로 변경
5. `npx tsc --noEmit` 실행
