...
- (참고: `pairing_confirm`에도 동일 패턴이 있었으나 v0.3.4에서 제거됨)
...
4. **broker 일부 후처리 누락**: broker.submitApproval()은 pending 제거, rejection 처리, signer revoke, wallet create/delete 등 대부분의 후처리를 이미 수행하지만, savePolicy와 trusted approver sync는 daemon에 남아있음 → daemon이 store에 직접 접근하는 레이어 위반 발생
...
2. **WDK 실패 이벤트 추가**: `ApprovalFailed` 이벤트를 guarded-wdk에 추가. 범위: `broker.submitApproval()` 내부 어디서든 실패 (검증 실패 + 후처리 실패 포함). 후처리를 broker 내부로 이동하므로 broker가 전체 실패를 책임짐
...
5. **이벤트 발행 시점을 원자적으로**: broker 내부에서 검증 + 후처리가 모두 성공한 뒤에만 성공 이벤트 발행. 중간 실패 시 `ApprovalFailed` 발행. `ApprovalVerified` → 후처리 실패 → 성공 이벤트 없이 `ApprovalFailed` 발행 가능
6. 변경 후: daemon→app 이벤트 경로가 **단일 경로, 대칭적** (성공도 실패도 WDK 이벤트) + daemon은 WDK facade만 사용
...
- 도메인 작업별 개별 실패 이벤트 (PolicyApplyFailed 등) — ApprovalFailed 하나로 통합 처리. 세분화는 필요 시 후속 Phase
...
- **ApprovalFailed 이벤트를 guarded-wdk에 추가**한다 (ControlResult의 에러 전달 역할 대체). 범위: broker.submitApproval 내부 전체 (검증 + 후처리). 후처리를 broker에 내재화하므로 broker가 전체 실패를 책임
...
| **protocol** | IN | event_stream wire type (`control.ts:135`)에 WDK 이벤트 타입 규격 반영 필요 |
