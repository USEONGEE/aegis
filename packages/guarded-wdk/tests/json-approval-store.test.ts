...
describe('JsonApprovalStore', () => {
  ...
  describe('history', () => {
    ...
    test('getHistory filters by seedId, type, chain', async () => {
      await store.appendHistory({ seedId: 's1', type: 'tx', chain: 'ethereum', targetHash: '0x1', approver: 'a', deviceId: 'd', action: 'approved' })
      await store.appendHistory({ seedId: 's1', type: 'policy', chain: 'ethereum', targetHash: '0x2', approver: 'a', deviceId: 'd', action: 'approved' })
      await store.appendHistory({ seedId: 's2', type: 'tx', chain: 'solana', targetHash: '0x3', approver: 'a', deviceId: 'd', action: 'rejected' })
    ...
    test('getHistory respects limit', async () => {
      for (let i = 0; i < 5; i++) {
        await store.appendHistory({ seedId: 's1', type: 'tx', chain: 'eth', targetHash: `0x${i}`, approver: 'a', deviceId: 'd', action: 'approved' })
      }
  ...
  describe('execution journal', () => {
    ...
    test('listJournal returns all entries', async () => {
      await store.saveJournalEntry({ intentId: 'i1', seedId: 's1', chain: 'eth', targetHash: '0x1', status: 'received' })
      await store.saveJournalEntry({ intentId: 'i2', seedId: 's1', chain: 'eth', targetHash: '0x2', status: 'settled' })
    ...
    test('listJournal filters by seedId, status, chain', async () => {
      await store.saveJournalEntry({ intentId: 'i1', seedId: 's1', chain: 'eth', targetHash: '0x1', status: 'received' })
      await store.saveJournalEntry({ intentId: 'i2', seedId: 's1', chain: 'eth', targetHash: '0x2', status: 'settled' })
      await store.saveJournalEntry({ intentId: 'i3', seedId: 's2', chain: 'sol', targetHash: '0x3', status: 'received' })
    ...
    test('listJournal respects limit', async () => {
      for (let i = 0; i < 5; i++) {
        await store.saveJournalEntry({ intentId: `i${i}`, seedId: 's1', chain: 'eth', targetHash: `0x${i}`, status: 'received' })
      }
